...
======================================
CLI script to evaluate Hybrid Retrieval (Dense + BM25 via Reciprocal Rank Fusion)
...
Saves machine-readable results to `results/hybrid_retrieval_eval.json`.
...
from app.retrieval.hybrid_service import HybridRetrievalService  # noqa: E402

logging.basicConfig(level=logging.WARNING, format="%(levelname)s: %(message)s")
logger = logging.getLogger(__name__)

QUESTIONS_PATH = _REPO_ROOT / "data" / "evaluation" / "questions.jsonl"
OUTPUT_PATH = RESULTS_DIR / "hybrid_retrieval_eval.json"
SEP = "=" * 40
...
def evaluate() -> int:
    ...
    try:
        ...
        print(f"ERROR: Failed to initialize hybrid retrieval service: {exc}")
    ...
    print(
        f"Running Hybrid RRF retrieval (top_k=10, candidate_k={DEFAULT_HYBRID_CANDIDATE_K}, rrf_k={DEFAULT_RRF_K})...\n"
    )
    ...
    print(f"\n{SEP}")
    print("HYBRID RETRIEVAL EVALUATION PASSED")
    print(f"{SEP}\n")
    return 0
