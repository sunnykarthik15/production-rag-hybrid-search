...
====================================
CLI script to evaluate dense vector retrieval performance against
...
Saves machine-readable results to `results/dense_retrieval_eval.json`.
...
from app.retrieval.service import DenseRetrievalService  # noqa: E402

logging.basicConfig(level=logging.WARNING, format="%(levelname)s: %(message)s")
logger = logging.getLogger(__name__)

QUESTIONS_PATH = _REPO_ROOT / "data" / "evaluation" / "questions.jsonl"
OUTPUT_PATH = RESULTS_DIR / "dense_retrieval_eval.json"
SEP = "=" * 40
...
def evaluate() -> int:
    ...
    try:
        ...
        print(f"ERROR: Failed to initialize retrieval service: {exc}")
    ...
    print("Running dense vector retrieval (top_k=10)...\n")
    ...
    for q in questions:
        response = service.retrieve(q.question, top_k=10)
    ...
    # Print clean evaluation report as required
    print(f"{SEP}")
    ...
    output_data = {
        ...
        "embedding_dimension": service.embedding_service.expected_dim,
    ...
    print(f"\n{SEP}")
    print("DENSE RETRIEVAL EVALUATION PASSED")
    print(f"{SEP}\n")
    return 0
