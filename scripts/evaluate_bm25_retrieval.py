#!/usr/bin/env python3
"""
scripts/evaluate_bm25_retrieval.py
===================================
CLI script to evaluate BM25 lexical retrieval performance against
the 150 evaluation questions in the Nexora dataset.
...
Saves machine-readable results to `results/bm25_retrieval_eval.json`.
...
from app.retrieval.bm25_service import BM25RetrievalService  # noqa: E402

logging.basicConfig(level=logging.WARNING, format="%(levelname)s: %(message)s")
logger = logging.getLogger(__name__)

QUESTIONS_PATH = _REPO_ROOT / "data" / "evaluation" / "questions.jsonl"
OUTPUT_PATH = RESULTS_DIR / "bm25_retrieval_eval.json"
SEP = "=" * 40


def evaluate() -> int:
    print(f"\n{SEP}")
    print("BM25 LEXICAL RETRIEVAL EVALUATION")
    print(f"{SEP}\n")
    ...
    # Load BM25 retrieval service
    try:
        service = BM25RetrievalService()
        service.initialize()
    except Exception as exc:
        print(f"ERROR: Failed to initialize BM25 retrieval service: {exc}")
        return 1

    # Load questions
    questions = list(load_questions_jsonl(QUESTIONS_PATH))
    print(f"Loaded {len(questions)} evaluation questions.")
    print("Running BM25 lexical retrieval (top_k=10)...\n")
    ...
    for q in questions:
        response = service.search(q.question, top_k=10)
    ...
    print(f"{SEP}")
    print("BM25 RETRIEVAL EVALUATION RESULTS")
    print(f"{SEP}\n")
    ...
    for qtype in [qt.value for qt in QuestionType]:
        if qtype in category_metrics:
            ...
            category_summary[qtype] = {
                "count": cat["total"],
                "recall_at_1": (sum(cat["recalls"][1]) / len(cat["recalls"][1])) if cat["recalls"][1] else 0.0,
                "recall_at_3": (sum(cat["recalls"][3]) / len(cat["recalls"][3])) if cat["recalls"][3] else 0.0,
                "recall_at_5": c_r5,
                "recall_at_10": (sum(cat["recalls"][10]) / len(cat["recalls"][10])) if cat["recalls"][10] else 0.0,
                "mrr": c_mrr,
            }
    ...
    # Save output JSON
    output_data = {
        "model_name": "rank-bm25/BM25Okapi",
    ...
    print(f"\n{SEP}")
    print("BM25 RETRIEVAL EVALUATION PASSED")
    print(f"{SEP}\n")
    return 0
